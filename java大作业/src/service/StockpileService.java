...
public class StockpileService {
    ...
    /**
     * 将外部单价与综合均价对比，返回对比结果字符串
     */
    public static String comparePrice(String username, String itemId, double externalUnitPrice) {
        double avg = getAveragePrice(username, itemId);
        double diff = externalUnitPrice - avg;
        if (avg == 0) return "暂无历史购买记录，无法比较";
        if (diff < 0) {
            return String.format("✅ 划算！外部单价 ¥%.2f < 囤货均价 ¥%.2f，便宜了 ¥%.2f (%.1f%%)",
                    externalUnitPrice, avg, -diff, -diff / avg * 100);
        } else if (diff > 0) {
            return String.format("❌ 不划算！外部单价 ¥%.2f > 囤货均价 ¥%.2f，贵了 ¥%.2f (%.1f%%)",
                    externalUnitPrice, avg, diff, diff / avg * 100);
        } else {
            return String.format("⚖️ 持平，外部单价 = 囤货均价 = ¥%.2f", avg);
        }
    }
